...
#    POS2 <- snp_asGeneticPos(CHR, POS, dir = "./omni_genetic_map")

    # Get genetic positions (cM) from locally cached genetic map files
    # to avoid automatic downloading by snp_asGeneticPos()

    map_file <- paste0(
    "./omni_genetic_map/chr",
    chr,
    ".OMNI.interpolated_genetic_map.gz"
    )

    if (!file.exists(map_file)) {
    stop(paste("Genetic map file not found:", map_file))
    }    

    genetic_map <- data.table::fread(map_file)

    # Ensure map positions are ordered
    genetic_map <- genetic_map[order(genetic_map[[2]])]
